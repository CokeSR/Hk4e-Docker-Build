...
def mdk_guest_login():
   if not get_config()["auth"]["enable_server_guest"]:
      return json_rsp_with_msg(define.RES_LOGIN_CANCEL, "游客系统已关闭", {}) # todo: this seems to bug client out if already logged-in as guest
   ...
   try:
      ...
      if not guest:
         ...
         if not user:
            ...
            return json_rsp_with_msg(define.RES_LOGIN_ERROR, "系统错误，请稍后再试", {}) # account for which guest binding exists is not in db?
...
def mdk_shield_api_login():
   try:
      ...
      if not user:
         return json_rsp_with_msg(define.RES_LOGIN_FAILED, "错误的用户名或密码", {})
...
def mdk_shield_api_verify():
   try:
      ...
      if token["device"] != request.headers.get('x-rpc-device_id'):
         return json_rsp_with_msg(define.RES_LOGIN_FAILED, "游戏账号信息缓存错误", {}) # token was initially created on different device; forbid it
      ...
      if not user:
         ...
         return json_rsp_with_msg(define.RES_LOGIN_ERROR, "系统错误，请稍后再试", {}) # account for which token exists is not in db?
...
def account_register():
   ...
   if request.method == 'POST':
      ...
      if user:
         ...
         flash(f"密码长度至少为 {get_config()['security']['min_password_len']} 字节", 'error')
      else:
         cursor.execute(
